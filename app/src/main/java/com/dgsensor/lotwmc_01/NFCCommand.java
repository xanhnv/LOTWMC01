...
public class NFCCommand {
    ...
    public static byte[] SendGetSystemInfoCommandCustom(Tag myTag, DataDevice ma) {
        ...
        if (response[9] == (byte) 0xE0 ||response[9] == (byte) 0x20  ) {
            if (response[8] == (byte) 0x02) //ST product
            {
                ...
                if (response[7] >= (byte) 0x04 && response[7] <= (byte) 0x07) {
                    ...
                    //ma.setProductName("M24LR01E");	This product doesn't exists
                    ma.setBasedOnTwoBytesAddress(false);
                    boolDeviceDetected = true;
                    ...
                    //ma.setProductName("M24LR02E");	This product doesn't exists
                    ma.setBasedOnTwoBytesAddress(false);
                    boolDeviceDetected = true;
                } else if (response[7] >= (byte) 0x54 && response[7] <= (byte) 0x57) {
                    //ma.setProductName("M24LR32E");	This product doesn't exists
                    ma.setBasedOnTwoBytesAddress(true);
                    boolDeviceDetected = true;
                    ...
                    //ma.setProductName("M24LR08E");	This product doesn't exists
                    ma.setBasedOnTwoBytesAddress(false);
                    boolDeviceDetected = true;
                } else if (response[7] >= (byte) 0x64 && response[7] <= (byte) 0x67) {
                    //ma.setProductName("M24LR128E");	This product doesn't exists
                    ma.setBasedOnTwoBytesAddress(true);
                    boolDeviceDetected = true;
                } else if (response[7] >= (byte) 0x6C && response[7] <= (byte) 0x6F) {
                    //ma.setProductName("M24LR256E");	This product doesn't exists
                    ma.setBasedOnTwoBytesAddress(true);
                    boolDeviceDetected = true;
    ...
    public static byte[] Send_several_ReadSingleBlockCommands(Tag myTag, byte[] StartAddress, byte[] bytNbBytesToRead, DataDevice ma) {
        ...
        do {
            ...
            while (temp == null || temp[0] == 1 && cpt <= 5) {
                temp = SendReadSingleBlockCommand(myTag, new byte[]{bytAddress[0], bytAddress[1]}, ma);
                cpt++;
            }
            cpt = 0;
    ...
    public static byte[] SendWriteMultipleBlockCommand(Tag myTag, byte[] StartAddress, byte[] DataToWrite, DataDevice ma) {
        ...
        for (int i = 0; i < NBByteToWrite; i = i + 4) {
            ...
            while ((response[0] == (byte) 0x01) && cpt <= 2) {
                response = SendWriteSingleBlockCommand(myTag, new byte[]{(byte) incrementAddressStart0, (byte) incrementAddressStart1}, new byte[]{fullByteArrayToWrite[i], fullByteArrayToWrite[i + 1], fullByteArrayToWrite[i + 2], fullByteArrayToWrite[i + 3]}, ma);
                cpt++;
            }
